...
import deepl
translator_deep = deepl.Translator(st.secret['deepl_key'])
#nltk.download('punkt')

if 'sen' not in st.session_state:
    st.session_state['sen'] = ""

if 'traduzione' not in st.session_state:
    st.session_state['traduzione'] = ""
...
def call_2():
    st.session_state['sen'] = st.session_state['traduzione']
...
if done:
        ...
        path = "/corpus_"+source_len+"_"+target_len+"_"+domain
        text = open(path, 'w')
